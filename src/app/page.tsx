...
function LivePanel({ open, title, type, content, fullContent, previewHtml, artifactType, onClose, onCopy, onOpenNew, onDownload }: {
  open: boolean;
  title: string;
  ...
  // Reset preview when new content arrives
  useEffect(() => { setShowPreview(false); }, [title]);

  if (!open) return null;

  const isHtmlArtifact = artifactType === 'html' && previewHtml;
  const canPreview = (type === 'preview' && previewHtml) || isHtmlArtifact;

  // Determine icon + label based on artifactType first, then panel type
  const typeEmoji =
    artifactType === 'html'  ? '🌐' :
    artifactType === 'code'  ? '⌨️' :
    artifactType === 'tests' ? '🧪' :
    artifactType === 'markdown' || artifactType === 'plan' ? '📄' :
    type === 'code' ? '🔧' : type === 'preview' ? '🌐' : type === 'report' ? '📄' : '🧠';

  const typeLabel =
    artifactType === 'html'  ? 'واجهة HTML' :
    artifactType === 'code'  ? 'كود TypeScript' :
    artifactType === 'tests' ? 'اختبارات' :
    artifactType === 'markdown' ? 'تحليل' :
    artifactType === 'plan'  ? 'خطة تنفيذ' :
    type === 'code' ? 'توليد الكود' : type === 'preview' ? 'المعاينة' : type === 'report' ? 'التقرير' : 'التخطيط';

  // Syntax highlight hint
  const isStreaming = fullContent.length === 0 || content.length < fullContent.length;

  return (
    <div className="fixed top-0 left-0 h-full z-40 pointer-events-auto" style={{ width: '500px', maxWidth: '95vw' }}>
      <div className="h-full flex flex-col bg-gray-950 text-gray-100 border-l border-gray-800 shadow-2xl">
        ...
        <div className="flex items-center justify-between px-3 py-2 border-b border-gray-800 bg-gray-900">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-xs font-bold text-cyan-400 shrink-0">{typeLabel}</span>
            <span className="text-[10px] text-gray-500 truncate">— {title}</span>
            {isStreaming && fullContent.length > 0 && (
              <span className="text-[9px] text-yellow-500 animate-pulse shrink-0">● جاري التوليد</span>
            )}
          </div>
        ...
        {canPreview && showPreview ? (
          <iframe
            srcDoc={isHtmlArtifact ? previewHtml : previewHtml}
          ...
          <pre
            ref={contentRef}
            className="flex-1 overflow-auto p-3 text-xs font-mono leading-relaxed whitespace-pre-wrap break-words"
            dir="ltr"
            style={{ background: '#0d1117', color: artifactType === 'code' || artifactType === 'tests' ? '#7ee787' : '#e6edf3' }}
          >
            {content}
        ...
        {/* Bottom bar */}
        <div className="flex items-center gap-1.5 px-3 py-2 border-t border-gray-800 bg-gray-900 flex-wrap">
          ...
          <button onClick={onDownload} className="px-2.5 py-1 rounded text-[10px] bg-gray-800 hover:bg-gray-700 text-gray-300 transition-colors">
            💾 {artifactType === 'html' ? 'تحميل HTML' : artifactType === 'code' || artifactType === 'tests' ? 'تحميل .ts' : 'تحميل .md'}
          </button>
          <div className="flex-1" />
          <span className="text-[9px] text-gray-600">{fullContent.length.toLocaleString()} حرف</span>
        </div>
      </div>
    </div>
  );
...
export default function MindFlowBrain() {
  ...
  const [livePanelArtifactType, setLivePanelArtifactType] = useState<string | undefined>(undefined);
  ...
  const executeTask = useCallback(async (taskId: string, taskLabel: string, parentNodeId: string) => {
    ...
    const projectNode = nodesRef.current.find(n => n.isProject)
      || nodesRef.current.find(n => n.id === parentNodeId);
    const allTaskLabels = projectNode?.projectTasks?.map(t => t.label) ?? [];

    // ─── Detect artifact type for panel icon ───────────────
    const lbl = taskLabel.toLowerCase();
    const isCode = /كود|برمجة|تطوير|component|api|دالة|function|backend|frontend/.test(lbl);
    const isUI   = /واجهة|تصميم|ui|ux|صفحة|page|layout/.test(lbl);
    const isTest = /اختبار|test|تجربة|فحص/.test(lbl);
    const panelType = isCode || isTest ? 'code' : isUI ? 'preview' : 'planning';

    // ─── Open live panel immediately ───────────────────────
    setLivePanelOpen(true);
    setLivePanelTitle(taskLabel);
    setLivePanelType(panelType);
    setLivePanelArtifactType(undefined);
    setLivePanelContent(`🔍 تحليل المهمة: "${taskLabel}"\n⚙️  نوع المخرج: ${isCode ? 'كود TypeScript' : isUI ? 'واجهة HTML' : isTest ? 'اختبارات' : 'وثيقة/خطة'}\n\n⏳ جاري الاتصال بالذكاء الاصطناعي...\n`);
    ...
    try {
      ...
      const res = await fetch('/api/brain', {
        ...
        body: JSON.stringify({
          ...
          projectContext: projectNode?.label || '',
          projectIdea: projectNode?.summary || projectNode?.label || '',
          allTasks: allTaskLabels,
        }),
      });
      ...
      const result       = data.result   || '✅ تم تنفيذ المهمة';
      const artifact     = data.artifact || '';
      const artifactType = data.artifactType || 'plan';

      // ─── Update Live Panel with full artifact (typing animation) ─
      const fullText = artifact || result;
      setLivePanelFullContent(fullText);
      setLivePanelArtifactType(artifactType);

      // Set preview HTML if it's an HTML artifact
      if (artifactType === 'html' && artifact) {
        setPreviewHtml(artifact);
      }

      let idx = 0;
      const chunkSize = fullText.length > 3000 ? 20 : 8;
      const typingInterval = setInterval(() => {
        idx = Math.min(idx + chunkSize, fullText.length);
        setLivePanelContent(fullText.slice(0, idx));
        if (idx >= fullText.length) clearInterval(typingInterval);
      }, 12);

      // ─── Mark task done on all matching nodes ───────────
      setNodes(prev => {
        const updated = prev.map(n => {
          const hasTask = n.projectTasks?.some(t => t.id === taskId || t.label === taskLabel);
          if (hasTask) {
            ...
            return { ...n, projectTasks: updatedTasks, projectProgress: progress, projectPhase: allDone ? 'done' as const : 'execution' as const };
          }
          // Store artifact in the task node's summary
          if (n.label === taskLabel && n.tag === 'مهمة') {
            return { ...n, summary: result, aiSummary: artifact.slice(0, 500) };
          }
          return n;
        });
        ...
        return projectRoot
          ? updateAncestorProgress(projectRoot.id, updated)
          : updateAncestorProgress(parentNodeId, updated);
      });
      ...
      const msg = `❌ خطأ في تنفيذ المهمة: ${err instanceof Error ? err.message : 'خطأ غير معروف'}\n\nتحقق من الاتصال بالخادم وأعد المحاولة.`;
      setLivePanelContent(msg);
      setLivePanelFullContent(msg);
  ...
  return (
    <div ref={containerRef} className="relative w-screen h-screen overflow-hidden" style={{ background: theme === 'dark' ? '#050508' : '#f0f0f5', pointerEvents: 'none' }}>
      ...
      <LivePanel
        ...
        artifactType={livePanelArtifactType}
        previewHtml={livePanelArtifactType === 'html' ? previewHtml : livePanelType === 'preview' ? previewHtml : ''}
        ...
        onDownload={() => {
          const ext =
            livePanelArtifactType === 'html'  ? 'html' :
            livePanelArtifactType === 'code'  ? 'ts' :
            livePanelArtifactType === 'tests' ? 'test.ts' :
            livePanelArtifactType === 'markdown' || livePanelArtifactType === 'plan' ? 'md' :
            livePanelType === 'preview' ? 'html' : livePanelType === 'code' ? 'ts' : 'md';
          const mime = ext.endsWith('html') ? 'text/html' : ext.endsWith('ts') ? 'text/plain' : 'text/markdown';
          const blob = new Blob([livePanelFullContent], { type: `${mime};charset=utf-8` });
          const link = document.createElement('a');
          link.download = `mindflow-${livePanelTitle.replace(/[^a-zA-Z0-9\u0600-\u06FF]/g, '-').slice(0, 30)}-${Date.now()}.${ext}`;
          link.href = URL.createObjectURL(blob);
          link.click();
          URL.revokeObjectURL(link.href);
        }}
